package com.cobol.serialization;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.dataformat.xml.annotation.JacksonXmlProperty;
import com.fasterxml.jackson.dataformat.xml.annotation.JacksonXmlRootElement;

/**
 * Java representation of the COBOL ws-record structure.
 * 
 * COBOL structure (from xml_generate.cbl and json_generate.cbl):
 * <pre>
 * 01  ws-record.
 *     05  ws-record-name                  pic x(10).
 *     05  ws-record-value                 pic x(10).
 *     05  ws-record-blank                 pic x(10).
 *     05  ws-record-flag                  pic x(5) value "false".
 *         88  ws-record-flag-enabled      value "true".
 *         88  ws-record-flag-disabled     value "false".
 * </pre>
 * 
 * Field mappings (from COBOL NAME OF clause):
 * - ws-record-name  -> "name"
 * - ws-record-value -> "value"
 * - ws-record-flag  -> "enabled"
 */
@JacksonXmlRootElement(localName = "ws-record")
@JsonInclude(JsonInclude.Include.NON_NULL)
public class Record {

    /**
     * Maps to ws-record-name (pic x(10))
     * Renamed to "name" in serialization output
     */
    @JsonProperty("name")
    @JacksonXmlProperty(localName = "name")
    private String name;

    /**
     * Maps to ws-record-value (pic x(10))
     * Renamed to "value" in serialization output
     */
    @JsonProperty("value")
    @JacksonXmlProperty(localName = "value")
    private String value;

    /**
     * Maps to ws-record-blank (pic x(10))
     * This field is suppressed when it contains only spaces (for XML)
     */
    @JsonProperty("ws-record-blank")
    @JacksonXmlProperty(localName = "ws-record-blank")
    private String blank;

    /**
     * Maps to ws-record-flag (pic x(5))
     * Renamed to "enabled" in serialization output
     * In XML, this becomes an attribute (type of ws-record-flag is attribute)
     * Values: "true" or "false" (matching 88-level conditions)
     */
    @JsonProperty("enabled")
    @JacksonXmlProperty(localName = "enabled", isAttribute = true)
    private String enabled;

    public Record() {
        this.enabled = "false";
    }

    public Record(String name, String value, String blank, boolean enabled) {
        this.name = name;
        this.value = value;
        this.blank = blank;
        this.enabled = enabled ? "true" : "false";
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getValue() {
        return value;
    }

    public void setValue(String value) {
        this.value = value;
    }

    public String getBlank() {
        return blank;
    }

    public void setBlank(String blank) {
        this.blank = blank;
    }

    public String getEnabled() {
        return enabled;
    }

    public void setEnabled(String enabled) {
        this.enabled = enabled;
    }

    public void setEnabled(boolean enabled) {
        this.enabled = enabled ? "true" : "false";
    }

    public boolean isEnabled() {
        return "true".equals(this.enabled);
    }

    @Override
    public String toString() {
        return "Record{" +
                "name='" + name + '\'' +
                ", value='" + value + '\'' +
                ", blank='" + blank + '\'' +
                ", enabled='" + enabled + '\'' +
                '}';
    }
}
